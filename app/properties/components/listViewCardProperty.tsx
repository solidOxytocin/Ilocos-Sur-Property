...
import { MATERIAL_ICON_NAMES, MaterialIconName } from '../constants/material-icon-names';
...
function FeatureIconsComponent ({item, index, length}: FeatureIconsComponentProps){
  const MAX_FEATURE = 4;
    const iconName: MaterialIconName = MATERIAL_ICON_NAMES.includes(item.icon as MaterialIconName)
    ...
    if(index ===  MAX_FEATURE && length > MAX_FEATURE){
      return (
       <View className="  bg-gray-200 rounded-lg m-1 px-2 py-1 flex-row gap-1">
          <MaterialCommunityIcons name="dots-horizontal" color="black"  size={20}/> <Text className="text-sm">+{length - MAX_FEATURE} more </Text>
        </View>
      );
    }
...
export function  ListViewCardProperty({property}:  ListViewCardPropertyProps) {
  ...
  return (
    <View className="flex-row bg-white rounded-lg m-2 shadow-md shadow-gray-300 px-3 py-2">
  <Image
        ...
        <View className='flex-col items-center'>
            <FlatList
                  ...
                  renderItem={({item, index}) => (
                    <FeatureIconsComponent item={item} index={index+1} length={property.features.length  }/>
                  )}
                  numColumns= {2}
